    ...
    public static GHCommit getCommit(SCMSource source, SCMRevision revision) throws CouldNotGetCommitDataException {
        ...
        if (hash == null) {
            throw new CouldNotGetCommitDataException(
                    "Unknown revision class [" + revision.getClass() + "] or null hash");
        }
        ...
        GitHub gitHub = null;
        try {
            gitHub = Connector.connect(ghSource.getApiUri(), Connector.lookupScanCredentials((Item) ghSource.getOwner(),
                    ghSource.getApiUri(), ghSource.getCredentialsId()));
