...
/**
 * A strategy for avoiding automatic builds for commits with messages that
 * contain a specific pattern<br>
 * This is similar to what hudson.plugins.git.extensions.impl.MessageExclusion
 * does.<br>
 */
public abstract class CommitMessageBranchBuildStrategy extends BranchBuildStrategy {
    ...
    public abstract String getMessage(SCMSource source, SCMHead head, SCMRevision revision)
            throws CouldNotGetCommitDataException;
    ...
    public boolean isAutomaticBuild(SCMSource source, SCMHead head, SCMRevision currRevision,
        ...
        String message = null;
        try {
            message = getMessage(source, head, currRevision);
        ...
        if (getCompiledPattern().matcher(message).find()) {
            ...
            LOGGER.info(
                    "Automatic build prevented for job [{}] because commit message [{}] " + "matched expression [{}]",
                    ownerDisplayName, message, pattern);
            return false;
        }
        return true;
    }
