...
public abstract class CommitAuthorBranchBuildStrategy extends BranchBuildStrategy {
    ...
    public abstract String getAuthor(SCMSource source, SCMHead head, SCMRevision revision)
    ...
    public boolean isAutomaticBuild(SCMSource source, SCMHead head, SCMRevision currRevision,
        ...
        String author = null;
        try {
            author = getAuthor(source, head, currRevision);
        ...
        if (getCompiledPattern().matcher(author).find()) {
            ...
            LOGGER.info(
                    "Automatic build prevented for job [{}] because commit author [{}] " + "matched expression [{}]",
                    ownerDisplayName, author, pattern);
            return false;
        }
        return true;
    }
